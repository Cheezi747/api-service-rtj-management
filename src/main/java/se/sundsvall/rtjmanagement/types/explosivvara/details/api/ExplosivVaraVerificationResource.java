...
@RestController
@Validated
@RequestMapping("/{municipalityId}/{namespace}/errands/{errandId}/explosiv-vara-details/verify")
@Tag(name = "Explosiv vara — verification",
	description = "Kör den automatiska fullständighetskontrollen (bilaga, explosiva varor, hanteringsplats) och returnerar routing-utfallet. Anropas av BPMN-verifieringsstegets worker; giltig endast när ärendets typeSlug är EXPLOSIV_VARA. Explosiv vara auto-godkänns aldrig — utfallet är NEEDS_SUPPLEMENT eller NEEDS_MANUAL_REVIEW.")
@ApiResponses(value = {
...
class ExplosivVaraVerificationResource {

	private final ExplosivVaraVerificationService service;

	ExplosivVaraVerificationResource(final ExplosivVaraVerificationService service) {
		this.service = service;
	}

	@PostMapping(produces = APPLICATION_JSON_VALUE)
	@Operation(summary = "Verifiera explosiv-vara-ansökan (fullständighetskontroll)", responses = {
		@ApiResponse(responseCode = "200", description = "Successful Operation", useReturnTypeSchema = true)
	})
	ResponseEntity<ExplosivVaraVerificationResult> verify(
