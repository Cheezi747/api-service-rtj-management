...
@RestController
@Validated
@RequestMapping("/{municipalityId}/{namespace}/explosiv-vara/applications")
@Tag(name = "Explosiv vara — ansökan",
	description = "Skickar in en komplett ansökan om tillstånd för explosiv vara i ETT multipart-anrop: JSON-delen `application` + en eller flera `files` (bilagor). Servern skapar errand + details + explosiva varor + stakeholders (företag/kontaktperson/personer) + bilagor atomiskt och startar processen sist.")
@ApiResponses(value = {
...
class ExplosivVaraApplicationResource {

	private static final String FILES_REQUIRED_MESSAGE = "At least one file (bilaga) is required";

	private final ExplosivVaraApplicationService service;

	ExplosivVaraApplicationResource(final ExplosivVaraApplicationService service) {
		this.service = service;
	}

	@PostMapping(consumes = MULTIPART_FORM_DATA_VALUE, produces = ALL_VALUE)
	@Operation(summary = "Skicka in komplett ansökan om tillstånd för explosiv vara", responses = {
