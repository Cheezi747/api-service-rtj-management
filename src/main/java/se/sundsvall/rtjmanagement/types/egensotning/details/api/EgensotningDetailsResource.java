...
@RestController
@Validated
@RequestMapping("/{municipalityId}/{namespace}/errands/{errandId}/egensotning-details")
@Tag(name = "Egensotning — details",
	description = "1:1 type-specific application data for EGENSOTNING errands. Upsert via PUT; only valid when the errand's typeSlug is EGENSOTNING.")
@ApiResponses(value = {
...
class EgensotningDetailsResource {

	private final EgensotningDetailsService service;

	EgensotningDetailsResource(final EgensotningDetailsService service) {
		this.service = service;
	}

	@PutMapping(consumes = APPLICATION_JSON_VALUE, produces = ALL_VALUE)
	@Operation(summary = "Upsert egensotning details", responses = {
	...
	ResponseEntity<Void> upsertDetails(
		...
		@Valid @NotNull @RequestBody final EgensotningDetails details) {
	...
	@Operation(summary = "Read egensotning details", responses = {
		@ApiResponse(responseCode = "200", description = "Successful Operation", useReturnTypeSchema = true),
		@ApiResponse(responseCode = "404", description = "Not Found", content = @Content(mediaType = APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = Problem.class)))
	})
	ResponseEntity<EgensotningDetails> readDetails(
	...
	@Operation(summary = "Delete egensotning details", responses = {
		@ApiResponse(responseCode = "204", description = "Successful operation", useReturnTypeSchema = true)
	})
