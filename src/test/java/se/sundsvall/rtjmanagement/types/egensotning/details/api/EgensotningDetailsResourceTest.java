...
class EgensotningDetailsResourceTest {
	...
	void upsertDetails() {
		webTestClient.put()
			...
			.bodyValue(EgensotningDetails.create().withPersonnummer("199001011234").withFastighetsbeteckning("Sundsvall Stenstaden 1:23"))
			.exchange()
			.expectStatus().isNoContent();

		verify(serviceMock).upsert(eq(MUNICIPALITY_ID), eq(NAMESPACE), eq(ERRAND_ID), any(EgensotningDetails.class));
	}

	@Test
	void readDetails() {
		when(serviceMock.read(MUNICIPALITY_ID, NAMESPACE, ERRAND_ID))
			.thenReturn(EgensotningDetails.create().withFastighetsbeteckning("Sundsvall Stenstaden 1:23").withLastOutcome("AUTO_APPROVE"));

		final var response = webTestClient.get()
			.uri(uri -> uri.path(PATH).build(Map.of("municipalityId", MUNICIPALITY_ID, "namespace", NAMESPACE, "errandId", ERRAND_ID)))
			.exchange()
			.expectStatus().isOk()
			.expectBody(EgensotningDetails.class)
			.returnResult()
			.getResponseBody();

		assertThat(response).isNotNull();
		assertThat(response.getFastighetsbeteckning()).isEqualTo("Sundsvall Stenstaden 1:23");
		assertThat(response.getLastOutcome()).isEqualTo("AUTO_APPROVE");
