...
class ExplosivVaraApplicationTest {

	@Test
	void gettersAndSetters() {
		final var products = List.of(ExplosivGoodsProduct.create().withHazardClass("1.1").withProductName("Dynamit"));
		final var personer = List.of(ExplosivApplicantPerson.create().withRole("RESPONSIBLE_PERSON").withFirstName("Anna"));
		...
		application.setCompanyName("Sprängbolaget AB");
		...
		application.setPersons(personer);
		...
		assertThat(application.getCompanyName()).isEqualTo("Sprängbolaget AB");
		...
		assertThat(application.getPersons()).isEqualTo(personer);
	}

	@Test
	void builderEqualsHashCodeToString() {
		final var products = List.of(ExplosivGoodsProduct.create().withHazardClass("1.1").withProductName("Dynamit"));
		final var one = ExplosivVaraApplication.create()
		...
		final var two = ExplosivVaraApplication.create()
			.withApplicantEmail("kontakt@foretaget.se")
			.withOrganizationNumber("5560123456")
			.withCompanyName("Sprängbolaget AB")
			.withAnlaggningTyp("NEW")
		...
		assertThat(one).isNotEqualTo(ExplosivVaraApplication.create());
	}
}
