...
class BrandfarligVaraDetailsResourceTest {
	...
	void upsertDetails() {
		webTestClient.put()
			...
			.bodyValue(BrandfarligVaraDetails.create().withVerksamhetstyp("RESTAURANT").withProxy(true))
			.exchange()
			.expectStatus().isNoContent();

		verify(serviceMock).upsert(eq(MUNICIPALITY_ID), eq(NAMESPACE), eq(ERRAND_ID), any(BrandfarligVaraDetails.class));
	}

	@Test
	void readDetails() {
		when(serviceMock.read(MUNICIPALITY_ID, NAMESPACE, ERRAND_ID))
			.thenReturn(BrandfarligVaraDetails.create().withVerksamhetstyp("RETAIL"));

		final var response = webTestClient.get()
			.uri(uri -> uri.path(PATH).build(Map.of("municipalityId", MUNICIPALITY_ID, "namespace", NAMESPACE, "errandId", ERRAND_ID)))
			.exchange()
			.expectStatus().isOk()
			.expectBody(BrandfarligVaraDetails.class)
			.returnResult()
			.getResponseBody();

		assertThat(response).isNotNull();
		assertThat(response.getVerksamhetstyp()).isEqualTo("RETAIL");
