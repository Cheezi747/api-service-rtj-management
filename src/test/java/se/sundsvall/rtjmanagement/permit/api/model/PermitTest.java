...
class PermitTest {
	...
	void testBuilderMethods() {
		...
		final var validUntil = LocalDate.of(2031, 9, 1);
		final var created = OffsetDateTime.now();
		final var modified = OffsetDateTime.now();

		final var result = Permit.create()
			.withId("id")
		...
		assertThat(result.getPermitType()).isEqualTo("BRANDFARLIG_VARA");
		assertThat(result.getValidFrom()).isEqualTo(validFrom);
		assertThat(result.getValidUntil()).isEqualTo(validUntil);
		assertThat(result.getConditions()).isEqualTo("villkor");
